...
with t5:
    ...
    if not api_key:
        ...
        with col_reco:
            ...
            with mc1:
                best = max(companii, key=lambda c: (df[c].iloc[-1] / df[c].iloc[0] - 1))
                best_ret = (df[best].iloc[-1] / df[best].iloc[0] - 1) * 100
                st.markdown(f"""
            ...
            with mc3:
                avg_beta = np.mean([df_b[c].mean() for c in companii if c in df_b.columns])
